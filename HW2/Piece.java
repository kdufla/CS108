...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		skirt=new int[width];
		for(int i=0;i<skirt.length;i++) skirt[i]=height;
	...
	*/
	...
	/**
	 ...
	 logic: every coordinate is changed according rotation matrix. 
	 after rotation largest y becomes smallest x which should
	 be 0, so after rotation method moves object right to point 0, 0.  
	 */
	public Piece computeNextRotation() {
		TPoint[] tansform=new TPoint[body.length];
		
		for(int i=0;i<body.length;i++){
			tansform[i]=new TPoint(0, 0);
			tansform[i].x=-body[i].y+height-1;
			tansform[i].y=body[i].x;
		}
		
		Piece ret=new Piece(tansform);
	...
	*/
	public boolean equals(Object obj) {
		...
		//body is already sorted
		return Arrays.equals(this.getBody(), other.body);
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		Piece curr=root;
		Piece next;
		//makes cyclic linked list
		while(true){
			next=curr.computeNextRotation();
			if(next.equals(root)){//end of first cycle
				curr.next=root;
				break;
			}else{
				curr.next=next;
